...
public class SanPhamChiTietServiceImpl {
    ...
    public List<SanPhamChiTiet> getAll2() {
        list.clear();
        try {
            String sql = "select ct.MaSPCT, TenSanPham, TenThuongHieu, TenChatLieu, TenMauSac, Size, ct.SoLuong \n"
                    + "from SanPhamChiTiet ct\n"
                    + "join SanPham sp on ct.MaSanpham = sp.MaSanPham\n"
                    + "join ThuongHieu th on ct.MaThuongHieu=th.MaThuonghieu\n"
                    + "join ChatLieu cl on ct.MaChatLieu = cl.MaChatLieu\n"
                    + "join MauSac ms on ct.MaMauSac = ms.MaMauSac\n"
                    + "join Size size on ct.MaSize = size.MaSize";
    ...
    public List<SanPhamChiTiet> searchByMa(String keywordspct) {
        String sql = " select  MaSPCT,MaSanPham, MaThuongHieu, MaChatLieu, MaMauSac, MaSize, SoLuong from SanPhamChiTiet  WHERE  MaSPCT= ?";
        list.clear();
        try {
            Connection con = DBConnect.getConnection();
